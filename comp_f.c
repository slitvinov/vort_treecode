/*  CVS:$Id: comp_f.c,v 1.2 2002/01/21 18:38:38 klindsay Exp $
...
#if ORD_CNT
   fprintf (stderr, "dir_cnt = %ld dir_lev = %.4f dir_part_cnt = %.4f\n p cnt ord_lev ord_part_cnt\n", dir_cnt,
	    (double) dir_level / (double) dir_cnt, (double) dir_part_cnt / (double) dir_cnt);
   for (i = 0; i < MAX_P; i++)
      if (!ord_cnt[i])
	 fprintf (stderr, "%d %ld\n", i + 1, 0L);
      else
	 fprintf (stderr, "%d %ld %.4f %.4f\n", i + 1, ord_cnt[i], (double) ord_level[i] / (double) ord_cnt[i],
		  (double) ord_part_cnt[i] / (double) ord_cnt[i]);
   fflush (stderr);
#endif

#if TREE_STATS
   fprintf (stderr, "tree stats: depth = %d cells = %d cp_ratio = %f\n", depth (root), tree_cell_cnt (root),
	    child_parent_ratio (root));
   v_avg = (double) v_tot / (double) N_tot;
   fprintf (stderr, "avg cells visits = %f std. dev. = %f\n", v_avg, sqrt ((double) v_sqr_tot / (double) N_tot - v_avg * v_avg));
   fflush (stderr);
#endif
...
static void
init_work (int sheet_cnt, sheet sheets[MAX_SHEET_CNT], work *scratch, long *N_tot)
{
   int sheet_ind;
   ...
   for (sheet_ind = 0; sheet_ind < sheet_cnt; sheet_ind++) {
	 ...
	 *N_tot += N;
	 ...
	 for (j = 0; j < N; scratch++, j++) {
	    ...
	    scratch->wght_nrm =
	       sqrt (scratch->v_wght.x1 * scratch->v_wght.x1 + scratch->v_wght.x2 * scratch->v_wght.x2 +
...
direct_sum (vect pos, REAL del2, tree_cell *cell_ptr)
{
   ...
   for (i = 0; i < lim; scratch++, i++) {
      ...
      nrm = sqrt (del2 + diff.x1 * diff.x1 + diff.x2 * diff.x2 + diff.x3 * diff.x3);
      mult = -1 / (nrm * nrm * nrm);
       ...
       * ans.x3 += mult * crss.x3;
       */
      ans.x1 += mult * (diff.x2 * scratch->v_wght.x3 - diff.x3 * scratch->v_wght.x2);
      ans.x2 += mult * (diff.x3 * scratch->v_wght.x1 - diff.x1 * scratch->v_wght.x3);
      ans.x3 += mult * (diff.x1 * scratch->v_wght.x2 - diff.x2 * scratch->v_wght.x1);
   }
   return ans;
}
...
static void
scalar_mmnts (int p, vect diff, REAL mmnts[MAX_P][MAX_P][MAX_P])
{
   int m1, m2, m3;
...
expansion (vect diff, REAL del2, int p, vect mmnts[MAX_P][MAX_P][MAX_P])
{
   ...
   for (m1 = 0, m1_1 = 1; m1 < p; m1_1++, m1++)
	 ...
	 for (m3 = 0, m3_1 = 1; m1 + m2 + m3 < p; m3_1++, m3++) {
	     ...
	     * ans.x3 += add.x3;
	     */
	    ans.x1 += vect_a.x2 * mmnts[m1][m2][m3].x3 - vect_a.x3 * mmnts[m1][m2][m3].x2;
	    ans.x2 += vect_a.x3 * mmnts[m1][m2][m3].x1 - vect_a.x1 * mmnts[m1][m2][m3].x3;
	    ans.x3 += vect_a.x1 * mmnts[m1][m2][m3].x2 - vect_a.x2 * mmnts[m1][m2][m3].x1;
	 }
   return ans;
}
...
comp_cff (REAL del2, vect diff, int p, REAL ans[MAX_P + 1][MAX_P + 1][MAX_P + 1])
{
   ...
   x22 = diff.x2 * 2;
   x32 = diff.x3 * 2;
   mult = -1 / (del2 + diff.x1 * diff.x1 + diff.x2 * diff.x2 + diff.x3 * diff.x3);
   ...
   for (i = 2; i <= p; i++) {
      ans[0][0][i] = (x32 * cff1[i] * ans[0][0][i - 1] + cff2[i] * ans[0][0][i - 2]) * mult;
      ans[0][i][0] = (x22 * cff1[i] * ans[0][i - 1][0] + cff2[i] * ans[0][i - 2][0]) * mult;
      ans[i][0][0] = (x12 * cff1[i] * ans[i - 1][0][0] + cff2[i] * ans[i - 2][0][0]) * mult;
   }
   ...
   for (i = 2; i < p; i++) {
      ans[0][1][i] = (diff.x2 * ans[0][0][i] + x32 * ans[0][1][i - 1] + ans[0][1][i - 2]) * mult;
      ans[1][0][i] = (diff.x1 * ans[0][0][i] + x32 * ans[1][0][i - 1] + ans[1][0][i - 2]) * mult;
      ans[0][i][1] = (x22 * ans[0][i - 1][1] + ans[0][i - 2][1] + diff.x3 * ans[0][i][0]) * mult;
      ans[1][i][0] = (diff.x1 * ans[0][i][0] + x22 * ans[1][i - 1][0] + ans[1][i - 2][0]) * mult;
      ans[i][1][0] = (x12 * ans[i - 1][1][0] + ans[i - 2][1][0] + diff.x2 * ans[i][0][0]) * mult;
      ans[i][0][1] = (x12 * ans[i - 1][0][1] + ans[i - 2][0][1] + diff.x3 * ans[i][0][0]) * mult;
   }
   ...
   for (i = 2; i <= p - 2; i++)
      for (j = 2; i + j <= p; j++) {
	 ans[0][i][j] =
	    (x22 * cff1[i] * ans[0][i - 1][j] + cff2[i] * ans[0][i - 2][j] + x32 * ans[0][i][j - 1] + ans[0][i][j - 2]) * mult;
	 ans[i][0][j] =
	    (x12 * cff1[i] * ans[i - 1][0][j] + cff2[i] * ans[i - 2][0][j] + x32 * ans[i][0][j - 1] + ans[i][0][j - 2]) * mult;
	 ans[i][j][0] =
	    (x12 * cff1[i] * ans[i - 1][j][0] + cff2[i] * ans[i - 2][j][0] + x22 * ans[i][j - 1][0] + ans[i][j - 2][0]) * mult;
      }

   /* two indices = 1, other index >= 1 */
   ans[1][1][1] = 5 * diff.x3 * mult * ans[1][1][0];
   for (i = 2; i <= p - 2; i++) {
      ans[1][1][i] = (diff.x1 * ans[0][1][i] + x22 * ans[1][0][i] + x32 * ans[1][1][i - 1] + ans[1][1][i - 2]) * mult;
      ans[1][i][1] = (diff.x1 * ans[0][i][1] + x22 * ans[1][i - 1][1] + ans[1][i - 2][1] + x32 * ans[1][i][0]) * mult;
      ans[i][1][1] = (x12 * ans[i - 1][1][1] + ans[i - 2][1][1] + diff.x2 * ans[i][0][1] + x32 * ans[i][1][0]) * mult;
   }
   ...
   for (i = 2; i <= p - 3; i++)
      for (j = 2; i + j < p; j++) {
	 ans[1][i][j] =
	    (diff.x1 * ans[0][i][j] + x22 * ans[1][i - 1][j] + ans[1][i - 2][j] + x32 * ans[1][i][j - 1] + ans[1][i][j - 2]) * mult;
	 ans[i][1][j] =
	    (x12 * ans[i - 1][1][j] + ans[i - 2][1][j] + diff.x2 * ans[i][0][j] + x32 * ans[i][1][j - 1] + ans[i][1][j - 2]) * mult;
	 ans[i][j][1] =
	    (x12 * ans[i - 1][j][1] + ans[i - 2][j][1] + x22 * ans[i][j - 1][1] + ans[i][j - 2][1] + diff.x3 * ans[i][j][0]) * mult;
      }
   ...
   for (i = 2; i <= p - 4; i++)
	 ...
	 for (k = 2; i + j + k <= p; k++)
	    ans[i][j][k] =
	       (x12 * cff1[i] * ans[i - 1][j][k] + cff2[i] * ans[i - 2][j][k] + x22 * ans[i][j - 1][k] + ans[i][j - 2][k] +
		x32 * ans[i][j][k - 1] + ans[i][j][k - 2]) * mult;
}
