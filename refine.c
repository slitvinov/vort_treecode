...
int refine(REAL eps_alpha2, REAL eps_th2, int sheet_cnt,
           sheet sheets[MAX_SHEET_CNT], work **scratch) {
  int i;
...
static int refine_fil(REAL eps2, fil *fil_ptr) {
  ...
  for (i = 0; i < N; i++) {
    ...
    n_ptr[i].ins_flag =
        diff.x1 * diff.x1 + diff.x2 * diff.x2 + diff.x3 * diff.x3 > eps2;
...
static int more_fils(sheet *sheet_ptr) {
  ...
  sheet_ptr->fil_alloc =
      (2 * sheet_ptr->fil_cnt / ARRAY_BLOCK + 1) * ARRAY_BLOCK;
