/*  CVS:$Id: main.c,v 1.2 2002/01/21 18:38:39 klindsay Exp $
...
static int
get_args (char **argv, FILE **infile, REAL *dtmin, REAL *eps_alpha2, REAL *eps_th2, REAL *stop, REAL *tol, int *node_min,
	  FILE **outfile)
{
   REAL eps;
...
rk_step (REAL dt, REAL del2, int sheet_cnt, sheet sheets[MAX_SHEET_CNT], work *scratch, REAL tol, int node_min, REAL *vel)
{
   ...
   for (sheet_ind = 0; sheet_ind < sheet_cnt; sheet_ind++) {
	 ...
	 for (j = 0; j < N; n_ptr++, j++) {
	    ...
	    REAL tmp = n_ptr->vel.x1 * n_ptr->vel.x1 + n_ptr->vel.x2 * n_ptr->vel.x2 + n_ptr->vel.x3 * n_ptr->vel.x3;
	    vel2 = MAX (vel2, tmp);
